...
class RoundImageView @JvmOverloads constructor(
    ...
    private fun drawLiftDown(canvas: Canvas) {
        ...
        path.arcTo(
            RectF(
                0f,
                (height - roundHeight * 2).toFloat(), (roundWidth * 2).toFloat(), height.toFloat()
            ), 90f, 90f
        )
        path.close()
        canvas.drawPath(path, paint!!)
    ...
    private fun drawRightUp(canvas: Canvas) {
        ...
        path.arcTo(
            RectF(
                (width - roundWidth * 2).toFloat(), 0f,
                width.toFloat(), (0 + roundHeight * 2).toFloat()
            ), -90f, 90f
        )
        path.close()
        canvas.drawPath(path, paint!!)
