...
import com.doubleclick.doctorapp.android.views.simplecropview.animation.SimpleValueAnimator
import com.doubleclick.doctorapp.android.views.simplecropview.animation.SimpleValueAnimatorListener
import com.doubleclick.doctorapp.android.views.simplecropview.animation.ValueAnimatorV14
import com.doubleclick.doctorapp.android.views.simplecropview.animation.ValueAnimatorV8
import com.doubleclick.doctorapp.android.views.simplecropview.callback.Callback
import com.doubleclick.doctorapp.android.views.simplecropview.callback.CropCallback
import com.doubleclick.doctorapp.android.views.simplecropview.callback.LoadCallback
import com.doubleclick.doctorapp.android.views.simplecropview.callback.SaveCallback
import com.doubleclick.doctorapp.android.views.simplecropview.util.Logger
import com.doubleclick.doctorapp.android.views.simplecropview.util.Utils
...
class CropImageView @JvmOverloads constructor(
    ...
    private var mCompressFormat: Bitmap.CompressFormat? = Bitmap.CompressFormat.PNG
    ...
    private fun drawOverlay(canvas: Canvas) {
        ...
        val overlayRect = RectF(
            Math.floor(
                mImageRect!!.left.toDouble()
            ).toFloat(), Math.floor(mImageRect!!.top.toDouble()).toFloat(), Math.ceil(
                mImageRect!!.right.toDouble()
            ).toFloat(), Math.ceil(mImageRect!!.bottom.toDouble()).toFloat()
        )
    ...
    private fun recalculateFrameRect(durationMillis: Int) {
        ...
        if (mIsAnimationEnabled) {
            val animator: SimpleValueAnimator? = animator
    ...
    private fun setupAnimatorIfNeeded() {
        if (mAnimator == null) {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                mAnimator = ValueAnimatorV8(mInterpolator)
            } else {
                mAnimator = ValueAnimatorV14(mInterpolator)
            }
        }
    }
        ...
        private get() {
            ...
            try {
                ...
                if (mAngle != 0f) {
                    ...
                    rotated.offset(
                        if (rotated.left < 0) originalImageWidth.toFloat() else 0.toFloat(),
                        if (rotated.top < 0) originalImageHeight.toFloat() else 0.toFloat()
                    )
                    cropRect = Rect(
                        rotated.left.toInt(),
                        rotated.top.toInt(),
    ...
    fun loadAsCompletable(
        ...
        return Completable.create(object : CompletableOnSubscribe {
            ...
            override fun subscribe(@NonNull emitter: CompletableEmitter) {
                mInitialFrameRect = initialFrameRect
                this@CropImageView.sourceUri = sourceUri
                if (useThumbnail) {
                    applyThumbnail(sourceUri)
        ...
        }).doOnSubscribe { mIsLoading.set(true) }.doFinally { mIsLoading.set(false) }
    }
    ...
    fun cropAsSingle(sourceUri: Uri? = null): Single<Bitmap?>? {
        ...
        }).doOnSubscribe { mIsCropping.set(true) }.doFinally { mIsCropping.set(false) }
    }
    ...
    fun saveAsSingle(bitmap: Bitmap?, saveUri: Uri): Single<Uri> {
        return Single.fromCallable { saveImage(bitmap, saveUri) }.doOnSubscribe {
            mIsSaving.set(
                true
            )
    ...
    class SavedState : BaseSavedState {
        ...
        private constructor(`in`: Parcel) : super(`in`) {
            ...
            compressFormat = `in`.readSerializable() as Bitmap.CompressFormat?
        ...
        companion object {
            val C: Parcelable.Creator<*> = object : Parcelable.Creator<Any> {
                override fun createFromParcel(p0: Parcel?): Any {
                    TODO("Not yet implemented")
                }

                override fun newArray(p0: Int): Array<Any> {
                    TODO("Not yet implemented")
                }
