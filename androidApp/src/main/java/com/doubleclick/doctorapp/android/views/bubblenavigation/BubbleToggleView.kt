...
import com.doubleclick.doctorapp.android.views.bubblenavigation.util.ViewUtils
...
class BubbleToggleView : RelativeLayout {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        var badgeTextColor = ContextCompat.getColor(context, R.color.white)
        ...
        if (icon == null) icon = ContextCompat.getDrawable(context, R.drawable.home)
    ...
    private fun createBubbleItemView(context: Context) {
        ...
        val lpIcon = LayoutParams(
            bubbleToggleItem!!.getIconWidth().toInt(), bubbleToggleItem!!.getIconHeight().toInt()
        )
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) lpTitle.addRule(
            END_OF, iconView!!.id
        ) else lpTitle.addRule(RIGHT_OF, iconView!!.id)
