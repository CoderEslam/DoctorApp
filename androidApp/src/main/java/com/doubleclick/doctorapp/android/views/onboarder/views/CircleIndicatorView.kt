package com.doubleclick.doctorapp.android.views.onboarder.views
...
class CircleIndicatorView : View {
    private lateinit var activeIndicatorPaint: Paint
    private lateinit var inactiveIndicatorPaint: Paint
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in 0 until indicatorsCount) {
            canvas.drawCircle(
                (radius + size * i).toFloat(), radius.toFloat(), (radius / 2).toFloat(),
                inactiveIndicatorPaint!!
            )
        }
        canvas.drawCircle(
            (radius + size * position).toFloat(), radius.toFloat(), (radius / 2).toFloat(),
